...
Repeat function for other mu and generating for multiple mu
```{r}
dataset_fn = function(mu) {
  dataset_df = rerun(5, sample_fn(n=30, mean=mu, sd= 5)) %>% 
    bind_rows %>%
    mutate(mu = mu)
...
mu_df = tibble(mu = c(1:6))

dataset_results = tibble(
  data = map(.x = mu_df, ~dataset_fn(mu = .x))
)
```
